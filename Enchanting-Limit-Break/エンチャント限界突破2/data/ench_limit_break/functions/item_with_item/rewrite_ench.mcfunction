...
execute store result entity @s[scores={Efficiency=1..}] Item.tag.Enchantments[{id:"minecraft:efficiency"}].lvl short 1 if score @s Efficiency <= MaxLevel Efficiency run scoreboard players get @s Efficiency
...
execute as @s[predicate=ench_limit_break:bow_or_crossbow] if score @s Power matches 0 if entity @e[nbt={Item:{tag:{Enchantments:[{id:"minecraft:power"}]}}},distance=0.001..1,tag=!ench_item] run tag @s add Enchanted
